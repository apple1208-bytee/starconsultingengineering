    ...
    >
      <div className="mx-auto flex h-16 max-w-[1280px] items-center justify-between px-6">
        ...
        <Link href="/" className="flex items-center gap-3 text-white">
          <img
            src="/Logo/flexotech_logo.png"
            alt="Flexotech Consulting Engineers Logo"
        ...
        <nav ref={menuRef} className="hidden items-center gap-1 lg:flex">
          <NavLink href="/" onClick={() => setOpenMenu(null)}>
            Home
          </NavLink>

          <NavLink href="/about" onClick={() => setOpenMenu(null)}>
            About
          </NavLink>
          ...
          <NavLink href="/tools" onClick={() => setOpenMenu(null)}>
            Tools
          </NavLink>

          <NavLink href="/blogs" onClick={() => setOpenMenu(null)}>
            Blogs
          </NavLink>

          <NavLink href="/careers" onClick={() => setOpenMenu(null)}>
            Careers
          </NavLink>
        </nav>
      ...
      {mobileOpen && (
        <div className="border-t border-navy-700 bg-navy-900 lg:hidden">
          <div className="space-y-1 px-6 py-4">
            ...
            <MobileLink href="/tools" onClick={() => setMobileOpen(false)}>
              Tools
            </MobileLink>

            <MobileLink href="/blogs" onClick={() => setMobileOpen(false)}>
              Blogs
            </MobileLink>

            <MobileLink href="/careers" onClick={() => setMobileOpen(false)}>
              Careers
            </MobileLink>
...
function MobileLink({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block rounded-md px-2 py-2 text-base font-medium text-white hover:bg-navy-800"
    >
      {children}
    </Link>
  );
...
function MobileGroup({
  ...
  return (
    <div>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between rounded-md px-2 py-2 text-base font-medium text-white hover:bg-navy-800"
      >
        {label}
        <ChevronDown
          size={16}
        ...
        <div className="space-y-1 pb-2 pl-4">
          {items.map((it) => (
            <Link
              ...
              href={it.href}
              onClick={onNavigate}
              className="block rounded-md px-2 py-1.5 text-sm text-steel-300 hover:text-white"
            >
              {it.label}
            </Link>
          ))}
